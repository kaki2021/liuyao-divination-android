(() => {
  ...
  box.replaceChildren();
  box.classList.add("analysis-monitor");
  box.setAttribute("aria-label", "本次分析进度");
  const title = el("h3", "analysis-progress-text", "准备分析"), meta = el("p", "analysis-job-model", "", "small-note");
  const stepText = el("p", "analysis-step-count", "", "small-note"), bar = el("progress", "analysis-stage-progress", "");
  bar.max = 4;
  bar.value = 0;
  bar.setAttribute("aria-label", "已完成的分析步骤");
  const list = el("ol", "analysis-stage-list", "");
  for (const [key, label] of stages) {
    const item = el("li", "analysis-stage-" + key, label);
    list.append(item);
  }
  const time = el("p", "analysis-elapsed", "", "small-note"), connection = el("p", "analysis-connection", "", "small-note");
  const hint = el("p", "analysis-wait-hint", "", "small-note"), error = el("p", "analysis-job-error", "", "error-text");
  error.hidden = true;
  ...
  const duration = (s) => {
    ...
    return s < 60 ? "".concat(s, " 秒") : "".concat(Math.floor(s / 60), " 分 ").concat(s % 60, " 秒");
  };
  function tick() {
    if (!job) return;
    const age = Math.floor((Date.now() - receivedAt) / 1e3), extra = running() ? age : 0;
    time.textContent = "本次已用 ".concat(duration((job.elapsed_seconds || 0) + extra)) + (running() && job.request_started_at ? " · 本次请求已等 ".concat(duration((job.request_elapsed_seconds || 0) + extra)) : "");
    connection.textContent = disconnected ? "状态连接中断：".concat(disconnected, " 正在自动重连，最后确认在 ").concat(new Date(receivedAt).toLocaleTimeString("zh-CN", { hour12: false }), "。") : "本地状态已连接 · ".concat(age < 3 ? "刚刚更新" : duration(age) + "前更新");
  }
  function update(value) {
    ...
    const failed = job.status === "failed", issue = job.error || ((_a = job.result) == null ? void 0 : _a.error);
    const fallback = ((_c = (_b = job.result) == null ? void 0 : _b.report_status) == null ? void 0 : _c.mode) === "fallback", preview = Boolean(job.preview_report);
    answer.hidden = !preview;
    title.textContent = active ? job.status === "queued" ? "任务已提交，等待开始" : "".concat(i >= 0 ? "第 " + (i + 1) + " / 4 步 · " : "").concat(name).concat(job.attempt === 2 ? " · 自动修复" : "") : failed ? "分析未完成".concat(i >= 0 ? " · 停在" + name : "") : ((_d = job.result) == null ? void 0 : _d.is_demo) ? "离线演示已完成" : job.status === "completed" ? "分析已完成" : "本次分析已结束";
    ...
    stepText.textContent = "已完成 ".concat(done.length, " / 4 步").concat(((_e = job.result) == null ? void 0 : _e.is_demo) ? " · 演示未调用 AI" : "");
    for (const [key] of stages) {
      const item = $("analysis-stage-" + key);
      item.dataset.state = done.includes(key) ? "done" : key === stage ? failed ? "error" : "current" : "pending";
    ...
    if (fallback) {
      ...
      stepText.textContent = "已完成 ".concat(done.length, " / 4 步 · 报告使用程序排版");
    }
    error.hidden = !issue;
    error.textContent = issue ? "".concat(issue.code || "分析错误", "：").concat(issue.message || "请查看本次回复与报错。") : "";
    note.textContent = active ? "分析期间暂不重复发起请求；刷新状态不会重新解卦或再次调用 AI。" : "现在可以重新解卦；新分析将另建记录。";
    box.dataset.status = failed ? "error" : active ? "running" : "finished";
    tick();
  }
  floating.onclick = () => {
    if ($("mobile-work")) $("mobile-work").click();
    $("tab-results").click();
  ...
  answer.onclick = () => {
    var _a;
    return (_a = callbacks.answer) == null ? void 0 : _a.call(callbacks);
  };
  refresh.onclick = async () => {
    var _a;
    refresh.disabled = true;
    try {
      await ((_a = callbacks.refresh) == null ? void 0 : _a.call(callbacks));
    } finally {
  ...
  window.AnalysisStatus = {
    ...
    start(id, caseId) {
      update({ job_id: id, case_id: caseId, status: "queued", stage: "queued", elapsed_seconds: 0, completed_stages: [] });
    },
    update,
    offline(message) {
      disconnected = message;
      box.dataset.status = "disconnected";
      tick();
    },
    clear() {
      job = null;
