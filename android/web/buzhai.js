(() => {
  ...
  const details = node("details", "", "person-details");
  details.id = "buzhai-details";
  ...
  const chooseGuide = node("details", "", "guide-details");
  chooseGuide.id = "buzhai-choice-guide";
  chooseGuide.append(node("summary", "四个选项怎么选？治理的三步怎么衔接？"), node("p", "择地／选房是一类主问，不要求每次走完四项。治风水则依《卜筮正术》044 依次核对范围 → 另问具体问题 → 再问整改方案；每步另有主问和实际起卦。你可以从当前已具备依据的阶段记录，但要交代前序判断，不能选一步就声称完成全流程。"));
  Object.values(stageHelp).forEach((g) => {
    const section = node("section", "", "guide-block");
    section.append(node("h4", g.name), node("p", g.when));
    chooseGuide.append(section);
  });
  $("buzhai-stage-help").before(chooseGuide);
  const roleHelp = node("details", "", "guide-details");
  roleHelp.append(node("summary", "场所类型、起卦人和入住情况怎么填？"), node("p", "阳宅用于居住或日常使用的场所；阴宅用于安葬相关场所。选择实际对象，不按问题吉凶来选。"), node("p", "起卦人填实际取得这一卦的人：使用者或主家亲自起卦选“本人”；卦师按自己的独立勘察起卦选“卦师”；替他人记录或角色尚不清楚选“其他”。请勿为了套用选房规则而改身份。"), node("p", "入住情况填起卦当时的事实。尚未入住只会在适用条件下提示相应原理，不会全局取消旬空扣分。阴宅或不涉及入住时选“未说明／不适用”。"), node("p", "“针对谁”填主要使用者或受益人；“地点”可用 A 房等代号。前序案例编号可留空，只用来关联记录，不会自动读取旧案内容。"));
  ...
  function update() {
    ...
    const help = $("buzhai-stage-help");
    help.replaceChildren(node("h4", "适合在什么情况下选"), node("p", g.when), node("h4", "这次回答什么"), node("p", g.ask), node("h4", "问法示例（不会自动填入案例）"), node("p", g.example), node("p", g.next, "small-note"));
  ...
  function read(validate = true) {
    ...
    for (const k of fields) {
      const input = $("buzhai-" + k);
      const text = input.value.trim();
      if (k === "previous_case_id" && !text) continue;
      value[k] = text;
      if (validate && ["beneficiary", "site", "proposal"].includes(k) && !text) {
        input.focus();
        throw new Error("请填写：" + document.querySelector('label[for="buzhai-' + k + '"]').textContent + "。");
      }
    }
    if (validate && value.previous_case_id && !/^case_[a-f0-9]{32}$/.test(value.previous_case_id)) {
      $("buzhai-previous_case_id").focus();
      throw new Error("前序案例编号应为 case_ 开头的完整编号；没有请留空。");
    }
    return value;
  }
  function fill(value) {
    $("buzhai-enabled").checked = Boolean(value);
    for (const k of fields) {
      const input = $("buzhai-" + k);
      input.value = (value == null ? void 0 : value[k]) || ({ stage: "site_choice", site_kind: "yang", role: "other", residence: "unknown" }[k] || "");
    }
    details.open = Boolean(value);
    update();
  }
  const canonical = (value) => value ? JSON.stringify(Object.fromEntries(fields.filter((k) => value[k] !== void 0).map((k) => [k, value[k]]))) : "";
  const panel = node("section", "", "paper-card topic-card");
  panel.id = "buzhai-result";
  panel.hidden = true;
  $("result-professional").prepend(panel);
  const flag = (value) => value === null || value === void 0 ? "待核对" : value ? "是" : "否";
  function render(chart) {
    const t = chart == null ? void 0 : chart.buzhai_analysis;
    panel.hidden = !t;
    ...
    d.open = true;
    d.append(node("summary", "五地候选与成立条件"));
    const f = t.five_lands;
    d.append(node("p", f.candidate_name + "（命名候选）", "topic-candidate"), node("p", f.condition), node("p", f.note, "small-note"));
    const lines = f.observations.map((l) => "".concat(l.layer === "hidden" ? "伏神" : "本卦", "第").concat(l.position, "爻").concat(l.branch, "：普通月令").concat(l.month_class || "未知", "，旬空").concat(flag(l.empty), "，月破").concat(flag(l.month_break)));
    d.append(node("p", lines.join("；") || "本卦及伏神未找到所需六亲。", "small-note"));
    panel.append(d);
    const r = t.residence_choice;
    if (r.applicable) {
      const choice = node("details");
      choice.append(node("summary", "选房条件核对"));
      const list = node("ul");
      ["全卦静：".concat(flag(r.all_static), "；动爻 ").concat(r.moving_count, " 个。静不等于旺。"), "六冲卦：".concat(flag(r.six_clash), "；六合卦：").concat(flag(r.six_combine), "。"), "世爻日冲：".concat(flag(r.shi_day_clash), "；实验暗动：").concat(flag(r.shi_hidden_moving_experimental), "。"), "世爻旬空：".concat(flag(r.shi_empty), "；普通月令：").concat(r.shi_month_class || "未知", "。"), r.empty_exception === "conditional_only" ? "尚未入住：可在世旺前提成立时讨论世空例外，目前不自动认定。" : "本次不启用“尚未入住且世旺”的旬空例外。"].forEach((s) => list.append(node("li", s)));
      choice.append(list, node("p", r.note, "small-note"));
      panel.append(choice);
    } else panel.append(node("p", "本次不是主家本人起卦的阳宅选房，不套用该场景的静旺、冲动和未入住世空偏好。", "small-note"));
    const limits = node("details");
    limits.append(node("summary", "出处与当前能力范围"), node("p", "《卜筮正术补充卜宅》043、044；《太卜风水术》PDF 第130、149～150页。全部原理可在规则库查看、导出。", "small-note"));
    t.limitations.forEach((s) => limits.append(node("p", s, "small-note")));
    panel.append(limits);
  }
  const dialog = $("rules-page"), parameter = node("div"), principles = node("div");
  parameter.id = "parameter-panel";
  principles.id = "principle-panel";
  ...
  for (const b of [parameterTab, principleTab]) {
    b.type = "button";
    tabs.append(b);
  }
  parameterTab.setAttribute("aria-pressed", "true");
  principleTab.setAttribute("aria-pressed", "false");
  dialog.append(tabs, parameter, principles);
  let guide = null;
  function show(which) {
    parameter.hidden = which !== "parameter";
    principles.hidden = which !== "principles";
    parameterTab.setAttribute("aria-pressed", String(which === "parameter"));
    principleTab.setAttribute("aria-pressed", String(which === "principles"));
  }
  parameterTab.addEventListener("click", () => show("parameter"));
  principleTab.addEventListener("click", async () => {
    show("principles");
    if (guide) return;
    principles.replaceChildren(node("p", "正在读取原理…"));
    try {
      const response = await fetch("/api/principles", { headers: { "X-App-Request": "1" } });
      if (!response.ok) throw new Error("原理未能读取，请重试。");
      guide = await response.json();
      principles.replaceChildren(node("p", guide.note, "topic-help"));
      const link = node("a", "导出原理及出处（JSON）", "secondary-button");
      link.href = "/api/principles/export";
      link.download = "卜宅原理.json";
      principles.append(link);
      const table = node("table", "", "principle-table");
      const head = node("tr");
      ["命名候选", "世爻六亲", "还须核对旺相的六亲"].forEach((t) => head.append(node("th", t)));
      table.append(head);
      guide.five_lands.forEach((r) => {
        const tr = node("tr");
        [r.name, r.shi_relative, r.required_relative].forEach((t) => tr.append(node("td", t)));
        table.append(tr);
      });
      principles.append(table);
      principles.append(node("p", "下面各条可展开。这里没有需要修改的数值；实验加减分仍在“评分参数”中单独说明。", "small-note"));
      guide.rules.forEach((r) => {
        const d = node("details", "", "principle-entry");
        d.append(node("summary", r.title), node("p", r.statement), node("p", "适用范围：" + r.scope, "small-note"));
        r.limitations.forEach((t) => d.append(node("p", "边界：" + t, "small-note")));
        const source = guide.sources.find((s) => s.source_id === r.source_id);
        d.append(node("p", "出处：《" + source.title + "》" + r.source_locator, "source-note"));
        principles.append(d);
      });
    } catch (e) {
      guide = null;
      principles.replaceChildren(node("p", e.message));
    }
  });
