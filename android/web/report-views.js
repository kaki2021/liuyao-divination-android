(() => {
  ...
  const records = el("section", "", "paper-card");
  records.append(el("h3", "分析过程与模型说明"));
  const model = el("div");
  ...
  function render(report, container, { isDemo = false } = {}) {
    ...
    if (plain) {
      ...
      for (const [key, label] of [["watch_for", "需要留意"], ["next_steps", "接下来怎么做"]]) {
        if (!((_a = plain[key]) == null ? void 0 : _a.length)) continue;
    ...
    if ((c == null ? void 0 : c.answer) && !isDemo) {
      const d = el("details");
      d.append(el("summary", "查看原始综合判断"), el("p", c.answer));
      professional.append(d);
    }
    if (sections.length) {
      let page = function(i) {
        current = i;
