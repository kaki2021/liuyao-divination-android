...
(() => {
  ...
  function readable(value) {
    ...
    if (typeof value === 'object') return readable(value.text ?? value.content ?? value.message ?? value.description ?? value.question ?? value.reason ?? value.summary ?? value.gap ?? '');
    return '';
  }
  ...
  function analysisLabel(run) {
    const index = state.runs.findIndex(item => item.analysis_run_id === run?.analysis_run_id);
    const date = new Date(run?.analyzed_at);
    const stamp = run?.analyzed_at && !Number.isNaN(date.valueOf()) ? date.toLocaleString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }) : '时间未记录';
    return `${index >= 0 ? `第 ${index + 1} 次分析` : '所选分析'} · ${stamp} · ${STATUS[run?.status] || '已记录'}`;
  }
  ...
  function getError(error) { return error && error.message ? error.message : '操作未完成，请稍后重试。'; }
  function uuid() {
    return window.LiuyaoCompat.requestId();
  }
  async function api(path, { method = 'GET', data } = {}) {
    const headers = { 'X-App-Request': '1', Accept: 'application/json' };
    if (method !== 'GET') { headers['Content-Type'] = 'application/json'; headers['X-Idempotency-Key'] = uuid(); }
    // XHR timeout works on older Android WebViews as well as current browsers.
    const response = await new Promise((resolve, reject) => {
      const request = new XMLHttpRequest(); request.open(method, path); request.timeout = 12000;
      ...
      request.send(data === undefined ? null : JSON.stringify(data));
    });
    let payload;
    try { payload = JSON.parse(response.text); } catch (_) { throw new Error('服务没有返回可读取的结果，请重试。'); }
    if (!response.ok || (payload.error && !payload.status)) {
      const error = new Error(payload.error?.message || '操作未完成，请重试。');
  ...
  function setBusy(value) {
    state.busy = value;
    document.querySelectorAll('#case-form button, #case-form input, #case-form textarea, #case-form select, #context-form button, #feedback-form button, #new-case, .history-item').forEach(el => { el.disabled = value; });
    updatePersonFields();
  ...
  function showView(view, { scroll = false } = {}) {
    ...
    ['input', 'results', 'feedback'].forEach(name => {
      const selected = name === state.view; const tab = $(`tab-${name}`);
  ...
  function createYao(stateValue, sizeClass = '') {
    ...
    const el = node('span', `yao ${sizeClass} ${!definition ? 'unknown' : ''} ${definition?.moving ? 'moving' : ''}`.trim());
  ...
  function buildInputs() {
    POSITIONS.forEach((label, index) => {
      ...
      fieldset.append(node('legend', 'sr-only', `第${index + 1}次，${label}`));
      STATES.forEach(definition => {
        const option = node('label', 'state-option');
        const radio = document.createElement('input');
        radio.type = 'radio'; radio.name = `line-${index + 1}`; radio.value = definition.value; radio.required = true;
        radio.setAttribute('aria-label', `${label}，${definition.label}${definition.moving ? '，动爻' : '，静爻'}`);
        const face = node('span', 'option-face');
        face.append(node('span', 'option-mark', definition.marker), node('span', '', definition.label));
        option.append(radio, face); fieldset.append(option);
        radio.addEventListener('change', onInputChange);
      });
      row.append(fieldset); $('line-inputs').append(row);
    });
    ['第一次 · 下卦', '第二次 · 上卦', '第三次 · 动爻'].forEach((label, index) => {
      ...
      MEIBU.forEach(definition => {
        const detail = index < 2 ? definition.trigram : definition.value === 'circle' ? '六爻全动' : definition.value === 'square' ? '六爻全静' : `${POSITIONS[Number(definition.value) - 1]}动`;
        options.append(makeCastingOption(`meibu-${index + 1}`, definition.value, definition.label, detail, `${label}，${definition.label}，${detail}`, 'meibu-option'));
      });
      fieldset.append(options); $('meibu-inputs').append(fieldset);
    });
    POSITIONS.forEach((label, index) => {
      const row = node('div', 'line-row taiji-row');
      row.append(node('span', 'line-label', `第${index + 1}次`));
      ...
      Object.entries(TAIJI).forEach(([combo, value]) => {
        ...
        fieldset.append(makeCastingOption(`taiji-${index + 1}`, combo, combo, `${definition.label}${definition.moving ? ' · 动' : ''}`, `第${index + 1}次，组合${combo}，${definition.label}`, 'taiji-option'));
      });
  ...
  function makeCastingOption(name, value, label, detail, accessibleLabel, extraClass) {
    const option = node('label', `state-option ${extraClass}`);
  ...
  function selectedLines() {
    ...
    if (method === 'direct') return POSITIONS.map((_, index) => document.querySelector(`input[name="line-${index + 1}"]:checked`)?.value || null);
  ...
  function canonicalPersonInfo(value) {
    ...
    if (value.querent_age !== undefined && value.querent_age !== null && value.querent_age !== '') result.querent_age = value.querent_age;
    if (subject !== 'self') {
      if (value.subject_age !== undefined && value.subject_age !== null && value.subject_age !== '') result.subject_age = value.subject_age;
    }
  ...
  function getPersonInfo(validate = true) {
    ...
    const result = { subject, background: $('person-background').value, relationship: $('person-relationship').value, profile_id: $('profile-select')?.value || '' };
    [['querent-age', 'querent_age', '问卦人'], ['subject-age', 'subject_age', '所问对象']].forEach(([id, key, label]) => {
      ...
      if (validate && (!Number.isInteger(age) || age < 0 || age > 150)) { $(id).focus(); throw new Error(`${label}的年龄请填写 0 至 150 的整数周岁，或留空。`); }
      result[key] = Number.isFinite(age) ? age : raw;
    });
    return canonicalPersonInfo(result);
  }
  function renderPreview() {
    const lines = selectedLines(); const view = window.LiuyaoChartDisplay.preview(lines);
    clear($('hex-preview-lines')); clear($('changed-preview-lines'));
    for (let index = 5; index >= 0; index--) {
      const definition = STATES.find(item => item.value === lines[index]);
      const row = node('div', 'preview-line');
      row.setAttribute('aria-label', `${POSITIONS[index]}：${definition ? definition.label : '未选择'}`);
      row.append(node('span', 'preview-position', index === 0 ? '初' : index === 5 ? '上' : String(index + 1)), createYao(lines[index]), node('span', 'motion-marker', definition?.moving ? definition.marker : ''));
      ...
      if (view.changed_structure) {
        ...
        const changed = node('div', `preview-line ${definition?.moving ? 'changed-position' : ''}`);
        changed.setAttribute('aria-label', `${POSITIONS[index]}：变后${view.changed_structure.bits[index] ? '阳爻' : '阴爻'}${definition?.moving ? '，由动爻变化' : ''}`);
        changed.append(node('span', 'preview-position', index === 0 ? '初' : index === 5 ? '上' : String(index + 1)), createYao(changedState), node('span', 'motion-marker', definition?.moving ? '←' : ''));
        $('changed-preview-lines').append(changed);
      }
    }
    $('preview-main-name').textContent = view.main.name;
    $('preview-changed-name').textContent = view.changed_structure?.name || (view.complete ? '静卦' : '待录入完整');
    ...
    $('preview-hint').textContent = !view.complete ? '完成选择后立即显示本卦与变卦，不需要先调用 AI。' : view.moving_positions.length ? `动爻：${view.moving_positions.map(position => POSITIONS[position - 1]).join('、')}。保存后可查看六亲、纳支和逐爻对照。` : '六爻皆静；保存后可查看本卦完整排盘。';
  }
  ...
  function getInput(validate = true) {
    ...
    if (validate && !question.trim()) { $('question').focus(); throw new Error('先写下这次想问的问题。'); }
    const method = castingMethod(); const input = { question };
    if (method === 'direct') {
      ...
      if (method === 'taiji' && state.originalCasting?.method === 'taiji') results = results.map((value, index) => taijiCombination(state.originalCasting.results[index]) === value ? state.originalCasting.results[index] : value);
    ...
    const buzhai = window.Buzhai?.read(validate);
  ...
  function inputsEqual(first, second) {
    ...
    if (window.Buzhai?.canonical(first.buzhai) !== window.Buzhai?.canonical(second.buzhai)) return false;
  ...
  function onInputChange() {
    renderPreview(); $('question-count').textContent = `${$('question').value.length} / 2000`;
    let changed = false;
    try { changed = state.caseId && !inputsEqual(getInput(false), state.input); } catch (_) { changed = Boolean(state.caseId); }
  ...
  function fillInput(input) {
    $('case-form').reset(); $('question').value = input?.question || '';
    window.Buzhai?.fill(input?.buzhai);
    const person = canonicalPersonInfo(input?.person_info);
    $('person-subject').value = person?.subject || 'unspecified';
    window.LiuyaoV5?.setProfile(person?.profile_id || '');
    $('querent-age').value = person?.querent_age === undefined ? '' : String(person.querent_age);
    $('subject-age').value = person?.subject_age === undefined ? '' : String(person.subject_age);
    $('person-relationship').value = person?.relationship || '';
    ...
    if (method !== 'direct') (input?.casting?.results || []).forEach((raw, index) => {
      ...
      if ((method === 'meibu' && MEIBU.some(item => item.value === value)) || (method === 'taiji' && TAIJI[value])) {
        const radio = document.querySelector(`input[name="${method}-${index + 1}"][value="${value}"]`); if (radio) radio.checked = true;
      }
    });
    (input?.lines || []).forEach((value, index) => {
      if (STATES.some(item => item.value === value)) {
        const radio = document.querySelector(`input[name="line-${index + 1}"][value="${value}"]`);
        if (radio) radio.checked = true;
      }
    });
    const rawTime = input?.actual_cast_time;
  ...
  function resetCase() {
    ...
    state.loadToken++; state.caseId = null; state.revision = 0; state.input = null; state.chart = null; state.runs = []; state.runId = null; state.pendingContext = null;
    resetRawReplies();
    ...
    banner(''); renderHistoryActive();
    showView('input'); $('question').focus();
  }
  function mayDiscard() {
    let dirty = false;
    try { dirty = hasEnteredData() && !inputsEqual(getInput(false), state.input); } catch (_) { dirty = true; }
    dirty = dirty || Boolean($('context-text').value.trim() || $('feedback-text').value.trim());
    return !dirty || window.confirm('还有未完成的输入，确定离开这次案例吗？');
  }
  async function refreshHistory() {
    const result = await api('/api/cases'); const list = $('history-list'); clear(list);
    if (!(result.cases || []).length) { list.append(node('p', 'empty-state', '还没有案例。第一卦，从这里开始。')); return; }
    const cases = [...result.cases].sort((a, b) => String(b.recorded_at || '').localeCompare(String(a.recorded_at || '')));
    cases.forEach(item => {
      const button = node('button', 'history-item'); button.type = 'button'; button.dataset.caseId = item.case_id; button.disabled = state.busy;
      button.append(node('span', 'history-question', item.question || '未命名的问题'));
      const meta = node('span', 'history-meta'); meta.append(node('span', '', dateText(item.recorded_at)), node('span', '', STATUS[item.status] || '已记录')); button.append(meta);
      button.addEventListener('click', () => { if (!state.busy && mayDiscard()) loadCase(item.case_id).catch(error => banner(getError(error), 'error')); }); list.append(button);
    });
    renderHistoryActive();
  }
  function renderHistoryActive() { document.querySelectorAll('.history-item').forEach(el => { el.classList.toggle('active', el.dataset.caseId === state.caseId); if (el.dataset.caseId === state.caseId) el.setAttribute('aria-current', 'true'); else el.removeAttribute('aria-current'); }); }
  function latestInput(record) { return record.revisions?.at(-1)?.input || record.original_input || record.input || {}; }
  async function loadCase(caseId, { keepForm = false, quiet = false, keepView = false } = {}) {
    const token = ++state.loadToken; const payload = await api(`/api/cases/${encodeURIComponent(caseId)}`);
    if (token !== state.loadToken) return;
    const record = payload.case || payload;
    if (state.caseId !== caseId) { window.AnalysisStatus.clear(); currentJob=null; $('context-text').value = ''; $('feedback-text').value = ''; state.pendingContext = null; }
    state.caseId = caseId; state.revision = record.revisions?.at(-1)?.revision_seq || payload.revision_seq || record.revision_seq || 1;
    state.input = latestInput(record); state.chart = payload.chart || record.chart; state.runs = record.analysis_runs || [];
    state.reportCurrent = payload.report_is_current !== false;
    if (!keepForm) fillInput(state.input);
    $('form-title').textContent = '回看这次所问'; $('case-state').textContent = '已保存'; $('results-section').hidden = false;
    $('export-case').href = `/api/cases/${encodeURIComponent(caseId)}/export`;
    $('export-diagnostics').href = `/api/cases/${encodeURIComponent(caseId)}/diagnostics`;
    renderChart(state.chart); renderEvents(record); renderRunSelector();
    const latestRun = state.runs.at(-1);
    if (payload.result) renderAnalysis(payload.result, latestRun?.status || payload.status || 'completed', latestRun?.analysis_run_id);
    else if (payload.display_report) renderAnalysis({ display_report: payload.display_report }, latestRun?.status || 'completed', latestRun?.analysis_run_id);
    else if (latestRun) renderRun(latestRun);
    else renderAnalysis(null, 'saved');
    onInputChange(); renderHistoryActive(); if (!quiet) banner('');
  ...
  function appendMiniChart(container, structure, label) {
    const block = node('div', 'chart-hex'); const picture = node('div', 'chart-mini');
    [...(structure.bits || [])].reverse().forEach(bit => picture.append(createYao(bit ? 'young_yang' : 'young_yin')));
    const words = node('div'); words.append(node('p', 'chart-description', label), node('p', 'chart-name', structure.name || '卦象'), node('p', 'chart-description', `${structure.upper_trigram || ''}上 · ${structure.lower_trigram || ''}下${structure.palace ? `　${structure.palace}宫${structure.palace_element || ''}` : ''}`));
    block.append(picture, words); container.append(block);
  }
  function renderChart(chart, input = state.input, scope = "当前已保存输入的排盘") {
    window.LiuyaoV5?.setAnalysisContext(input);
    window.Buzhai?.render(chart);
    $('result-question').textContent = input?.question || '这次的排盘与解卦';
    clear($('chart-summary')); clear($('chart-lines'));
    $('chart-context').textContent = scope;
    $('chart-question').textContent = input?.question || ''; $('chart-question').hidden = !input?.question;
    renderPersonSummary(input?.person_info);
    window.LiuyaoV5?.renderComprehensive(chart);
    renderFoundation(chart?.basic_analysis);
    ...
    if (chart.changed_structure) { $('chart-summary').append(node('span', 'chart-arrow', '→')); appendMiniChart($('chart-summary'), chart.changed_structure, '变卦'); }
    else { const staticBlock = node('div', 'chart-static'); staticBlock.append(node('p', 'chart-description', '变卦'), node('p', 'chart-name', '静卦'), node('p', '', '六爻皆静，无变卦')); $('chart-summary').append(node('span', 'chart-arrow', '→'), staticBlock); }
    [...(chart.main.lines || [])].sort((a, b) => b.position - a.position).forEach(line => {
      const row = node('tr'); row.append(node('td', '', POSITIONS[line.position - 1] || `${line.position}爻`), node('td', 'six-spirit-cell', line.six_spirit ? line.six_spirit.replace('腾蛇', '螣蛇') : '—'));
      const yaoCell = node('td'); const graphic = node('span', 'table-yao'); const definition = STATES.find(item => item.value === line.state);
      graphic.append(createYao(line.state), node('span', 'motion-marker', definition?.moving ? definition.marker : '')); graphic.setAttribute('aria-label', definition?.label || ''); yaoCell.append(graphic); row.append(yaoCell);
      row.append(node('td', '', `${line.branch || ''}${line.element || ''}`), node('td', '', line.relative || '—'));
      const tags = node('td');
      ...
      if (!tags.childElementCount) tags.append(node('span', 'small-note', '—')); row.append(tags);
      row.classList.toggle('moving-row', Boolean(line.moving));
      row.append(node('td', 'line-transition', line.moving ? `${line.yang ? '阳变阴' : '阴变阳'} →` : '—'));
      const changedCell = node('td', 'changed-yao-cell'); const changedBranch = node('td');
      if (chart.changed_structure) {
        const index = line.position - 1; const yang = Boolean(chart.changed_structure.bits[index]);
        const changedGraphic = node('span', 'table-yao'); changedGraphic.setAttribute('aria-label', `变卦${POSITIONS[index]}，${yang ? '阳爻' : '阴爻'}`);
        changedGraphic.append(createYao(yang ? 'young_yang' : 'young_yin')); changedCell.append(changedGraphic);
        const branch = chart.changed_structure.branches?.[index] || ''; changedBranch.textContent = branch + (window.LiuyaoChartDisplay.branchElement(branch) || '');
      } else { changedCell.textContent = '—'; changedBranch.textContent = '—'; }
      row.append(changedCell, changedBranch); $('chart-lines').append(row);
    });
    window.LiuyaoPhoneChart.render(chart);
    const bodyText = chart.gua_body_branch ? `卦身为${chart.gua_body_branch}${chart.gua_body_positions?.length ? '，对应爻位已标出' : '，本卦未见对应地支'}。` : '';
    document.querySelector('.chart-note').textContent = `${bodyText} ${chart.changed_structure ? '变卦整卦重新纳支，仅本卦动爻发生阴阳变化；动爻的变爻六亲按本卦宫五行确定。' : '六爻皆静，无变卦。'} 世应及身位标记属于本卦。`;
  }
  function renderPersonSummary(value) {
    const container = $('chart-person-info'); clear(container);
    const person = canonicalPersonInfo(value); container.hidden = !person;
    if (!person) return;
    const subjectLabels = { self: '问自己', other: '问他人', unspecified: '暂不指定或涉及多人' };
    const parts = [subjectLabels[person.subject]];
    if (person.querent_age !== undefined) parts.push(`问卦人起卦时 ${person.querent_age} 周岁`);
    if (person.subject_age !== undefined) parts.push(`所问对象起卦时 ${person.subject_age} 周岁`);
    if (person.relationship) parts.push(`关系：${person.relationship}`);
  ...
  function renderFoundation(analysis) {
    const container = $('foundation-content'); clear(container);
    ...
    sections.forEach((section, index) => {
      ...
      block.append(node('summary', '', `${section.title || '基础关系'}${items.length > 6 ? `（${items.length} 项）` : ''}`));
  ...
  function renderChartContext(chart) {
    const summary = $('calendar-summary'); clear(summary);
    const calendar = chart?.calendar; const computed = calendar?.status === 'computed';
    if (computed) {
      const pillars = node('dl', 'four-pillars');
      [['year', '年柱'], ['month', '月柱'], ['day', '日柱'], ['hour', '时柱']].forEach(([key, label]) => {
        const pair = node('div'); pair.append(node('dt', '', label), node('dd', '', calendar.pillars?.[key] || '—')); pillars.append(pair);
      });
      summary.append(pillars);
      if (calendar.cast_time) summary.append(node('p', 'calendar-time', `起卦时间：${calendar.cast_time.slice(0, 19).replace('T', ' ')} · 北京时间（UTC+08:00）`));
    } else summary.append(node('p', 'small-note', calendar?.message || '这份记录尚无四柱与六神资料。'));
    $('calendar-details').hidden = !calendar?.convention_label;
    $('calendar-convention').textContent = calendar?.convention_label || '';
    const bodyLine = chart?.main?.lines?.find(line => line.is_shi_body || line.position === chart.shi_body_position);
    const bodyPosition = bodyLine ? POSITIONS[bodyLine.position - 1] : null;
    const guaPositions = chart?.gua_body_positions || [];
    $('body-summary').hidden = !chart?.main;
    $('body-summary').textContent = chart?.main ? `世身：${bodyPosition || '未确定'}${bodyLine?.branch ? ` · ${bodyLine.branch}${bodyLine.element || ''}` : ''}　卦身：${chart.gua_body_branch || '未确定'}${guaPositions.length ? ` · ${guaPositions.map(position => POSITIONS[position - 1]).join('、')}` : chart.gua_body_branch ? ' · 本卦未见对应地支' : ''}` : '';
  }
  function appendListBlock(container, items, title, className) {
    ...
    const block = node('div', className); block.append(node('h4', '', title)); const list = node('ul'); values.forEach(text => list.append(node('li', '', text))); block.append(list); container.append(block);
  }
  function renderConclusion(container, conclusion) {
    ...
    const card = node('section', `conclusion-card conclusion-${direction}`);
  ...
  function renderAnalysisError(container, error) {
    ...
    block.append(node('h3', '', stage ? `${stage}阶段未完成` : '这次分析未完成'));
    ...
    if (detail.validation_detail || detail.code || detail.stage) {
      ...
      const lines = [detail.code && `错误代码：${detail.code}`, detail.stage && `分析阶段：${detail.stage}`, detail.attempts && `尝试次数：${detail.attempts}`, readable(detail.validation_detail)].filter(Boolean);
      technical.append(node('pre', '', lines.join('\n'))); block.append(technical);
    }
    container.append(block);
  }
  function resetRawReplies() {
    state.rawReplyToken++; state.rawReplyLoaded = false;
    const details = $('ai-raw-replies');
    if (details.dataset.runId !== (state.runId || '')) details.open = false;
    details.dataset.runId = state.runId || ''; details.hidden = !state.runId;
    clear($('ai-raw-replies-content'));
    const run = state.runs.find(item => item.analysis_run_id === state.runId);
    $('ai-raw-replies-context').textContent = state.runId ? `当前查看：${run ? analysisLabel(run) : '所选分析'}。${state.runs.length > 1 ? `本案例共 ${state.runs.length} 次分析，可通过“回看分析”切换。` : ''}` : '';
  }
  function appendSelectionDraft(container, attempt, raw) {
    ...
    field('AI 自报状态', Object.hasOwn(statuses, rawStatus) ? `${statuses[rawStatus]}（${rawStatus}）` : rawStatus);
    ...
    field('AI 填写的主候选编号', selected ? `${selected}${candidates.some(candidate => candidate.candidate_id === selected) ? '' : '（候选列表中没有这个编号）'}` : '未填写');
    draft.append(fields);
    ...
    candidates.forEach(candidate => {
      const card = node('div', 'ai-candidate-draft'); const label = typeof candidate.candidate_id === 'string' ? candidate.candidate_id : '未填写编号';
      ...
      add('对象', candidate.object_role); add('作用', candidate.function);
      ...
      card.append(entries); appendListBlock(card, candidate.assumptions, '假定条件', 'small-note'); draft.append(card);
    });
  ...
  async function loadRawReplies(force = false) {
    ...
    if (rawLoading) { if (force) rawReloadPending = true; return; }
    if (state.rawReplyLoaded && !force) return;
    rawLoading = true; lastRawCheck = Date.now();
    const token = ++state.rawReplyToken; const caseId = state.caseId; const runId = state.runId;
    const container = $('ai-raw-replies-content');
    if (!state.rawReplyLoaded) { clear(container); container.append(node('p', 'small-note', '正在读取这次分析的回复…')); }
    try {
      const payload = await api(`/api/cases/${encodeURIComponent(caseId)}/ai-attempts?run_id=${encodeURIComponent(runId)}`);
      ...
      rawSignature = signature; clear(container); state.rawReplyLoaded = true;
      const attempts = payload.analysis_runs?.find(run => run.analysis_run_id === runId)?.attempts || [];
      if (!attempts.length) { container.append(node('p', 'small-note', '尚未收到完整回复。AI 正在处理时不会逐字显示；每一步返回后这里自动更新。若调用失败，这里会显示报错。')); return; }
      const stages = { intent: '理解问题', selection: '确定取用', use_spirit: '确定取用', interpretation: '解卦', report: '整理报告' };
      attempts.forEach(attempt => {
        const item = node('section', 'ai-raw-attempt'); const heading = node('div', 'ai-raw-heading');
        const attemptLabel = attempt.attempt_number === 1 ? '首次生成' : attempt.attempt_number === 2 ? '自动修复' : `回复 ${attempt.attempt_number || '未编号'}`;
        heading.append(node('h4', '', `${stages[attempt.stage] || attempt.stage || '分析'} · ${attemptLabel}`));
        heading.append(node('span', `soft-tag ${attempt.adopted ? '' : 'warning'}`, attempt.adopted ? '已通过校验' : '未通过校验的草稿'));
        item.append(heading);
        const timing = [attempt.http_status ? 'HTTP '+attempt.http_status : '', Number.isFinite(attempt.elapsed_ms) ? '本次调用 '+(attempt.elapsed_ms/1000).toFixed(1)+' 秒' : ''].filter(Boolean);
        ...
        if (errors.length) item.append(node('p', 'ai-raw-errors', `未采用原因：${errors.join('\n')}`));
        if (attempt.normalization_changes?.length) {
          const changes = node('details', 'ai-normalization'); changes.append(node('summary', '', '程序对回复的整理记录'), node('p', '', attempt.normalization_changes.join('\n'))); item.append(changes);
        }
        const raw = typeof attempt.raw_output === 'string' ? attempt.raw_output : '';
        appendSelectionDraft(item, attempt, raw);
        const text = node('pre', 'ai-raw-text', raw || '没有返回正文。'); text.tabIndex = 0;
        text.setAttribute('aria-label', `${stages[attempt.stage] || 'AI'}原始回复`); item.append(text);
        if (raw) {
          const copy = node('button', 'secondary-button', '复制原始回复'); copy.type = 'button';
          const status = node('span', 'small-note ai-copy-status'); status.setAttribute('role', 'status');
          copy.addEventListener('click', async () => {
            try {
              if (!window.navigator?.clipboard?.writeText) throw new Error('clipboard unavailable');
              await window.navigator.clipboard.writeText(raw); status.textContent = '已复制。';
            } catch (_) { status.textContent = '浏览器未允许复制，请在上方正文中手动选取并复制。'; text.focus(); }
          });
      ...
      if (token !== state.rawReplyToken || caseId !== state.caseId || runId !== state.runId) return;
      clear(container); container.append(node('p', 'ai-raw-errors', `${getError(error)} 收起后重新展开可重试。`));
    } finally {
      rawLoading = false;
      if (rawReloadPending) { rawReloadPending = false; loadRawReplies(true); }
    }
  }
  function renderAnalysis(result, status = 'saved', runId = null, notice = '') {
    ...
    if (runId) {
      ...
      const snapshotChart = run?.outcome?.result?.chart_snapshot || run?.outcome?.result?.report?.chart || result?.chart || null;
      renderChart(snapshotChart, run?.input_snapshot?.input || null, `所选分析当时的排盘${run?.analyzed_at ? ' · ' + dateText(run.analyzed_at) : ''}`);
    } else renderChart(state.chart, state.input);
    const container = $('analysis-content'); clear(container); window.ReportViews.reset();
    if (notice) container.append(node('p', 'demo-banner', notice));
    if (result?.report_status?.mode === 'fallback') container.append(node('p', 'demo-banner', result.report_status.message));
    const tag = $('analysis-status'); tag.textContent = STATUS[status] || '已记录'; tag.className = `soft-tag ${status === 'failed' ? 'error' : ['partial', 'unresolved'].includes(status) ? 'warning' : ''}`;
    const historical = runId && (!state.reportCurrent || runId !== state.runs.at(-1)?.analysis_run_id);
    ...
    const duplicateFailureReport = Boolean(result.error && status === 'failed' && !report?.conclusion &&
      (typeof report === 'string' ? report === errorMessage : report && !report.sections?.length));
    const conclusion = !result.is_demo && report && typeof report === 'object' && readable(report.conclusion?.answer) ? report.conclusion : null;
    ...
    window.LiuyaoV5?.renderAudit(result);
    if (!hasReport && !result.clarifying_questions?.length && !result.unresolved?.length && !result.error) container.append(node('p', 'empty-state', status === 'failed' ? '本次分析未完成，输入与排盘已保留。可以重试或更换模型。' : '尚未形成可展示的完整报告。输入与本次分析过程已经保留。'));
  }
  function renderRun(run, { notice = '' } = {}) {
    const stored = run.outcome?.result || run.result || null;
    const result = stored?.report && (stored.report.display_report || stored.report.stage_outputs || stored.report.is_demo !== undefined) ? stored.report : stored;
    renderAnalysis(result, run.status || run.outcome?.status || 'running', run.analysis_run_id, notice);
  }
  function renderRunSelector() {
    const area = $('run-history'); clear(area); area.hidden = state.runs.length < 2;
    runSelect = null;
    if (state.runs.length < 2) return;
    const label = node('label', '', '回看分析'); label.htmlFor = 'past-run-select';
    const select = document.createElement('select'); select.id = 'past-run-select'; select.disabled = state.busy;
    runSelect = select;
    [...state.runs].reverse().forEach((run, index) => { const option = node('option', '', `${index === 0 ? '最近 · ' : ''}${analysisLabel(run)}`); option.value = run.analysis_run_id; select.append(option); });
    select.value = state.runs.some(run => run.analysis_run_id === state.runId) ? state.runId : state.runs.at(-1).analysis_run_id;
    select.addEventListener('change', () => { if (state.busy) return; window.AnalysisStatus.clear(); currentJob=null; const run = state.runs.find(item => item.analysis_run_id === select.value); if (run) renderRun(run); });
  ...
  async function saveCase() {
    ...
    if (state.caseId) {
      ...
      if (!reason.trim()) { $('revision-fields').hidden = false; $('revision-reason').focus(); throw new Error('请写一句修改原因，便于以后回看。'); }
      payload = await api(`/api/cases/${encodeURIComponent(state.caseId)}/revisions`, { method: 'POST', data: { input, reason, expected_revision_seq: state.revision } });
    } else payload = await api('/api/cases', { method: 'POST', data: { input } });
    state.caseId = payload.case_id || state.caseId; state.revision = payload.revision_seq || state.revision + 1; state.input = input; $('revision-reason').value = '';
    if (!state.caseId) throw new Error('案例没有成功保存，请重试。');
    await loadCase(state.caseId, { quiet: true }); await refreshHistory(); return state.caseId;
  }
  function chosenProvider() { return state.config?.providers?.find(item => item.id === $('provider-select').value); }
  function modelSettings() { return { provider: $('provider-select').value, model: $('model-select').value }; }
  function storePreference() {
    try { const preference=JSON.stringify({ ...modelSettings(), defaults: 'deepseek-pro-max' });localStorage.setItem('liuyao-model-preference', preference);window.LiuyaoAndroid?.savePreference(preference); } catch (_) { /* Selection still works for this page. */ }
  }
  function updateModelLabel() {
    const provider = chosenProvider();
    const quality = provider?.reasoning_effort;
    $('model-current').textContent = provider ? `${provider.name} · ${$('model-select').value || '未配置模型'}${quality ? ' · 质量 '+quality : ''}${provider.configured || provider.id === 'demo' ? '' : ' · 待配置'}` : '模型设置暂不可用';
    $('model-quality').hidden = !quality;
    $('model-quality').textContent = quality ? `思考质量：${quality}${provider.thinking === 'enabled' ? ' · 深度思考已启用' : ' · 思考已关闭'}` : '';
  }
  function updateModels(preferredModel) {
    const provider = chosenProvider(); const select = $('model-select'); clear(select);
    if (!provider) return;
    const models = provider.models?.length ? provider.models : provider.default_model ? [provider.default_model] : [];
    models.forEach(model => { const value = typeof model === 'string' ? model : model.id; if (!value) return; const option = node('option', '', typeof model === 'string' ? model : model.name || model.id); option.value = value; select.append(option); });
    if (!models.length) { const option = node('option', '', '尚未配置模型'); option.value = ''; select.append(option); }
    ...
    const note = $('provider-note'); note.className = `provider-note ${!provider.configured || provider.id === 'demo' ? 'warning' : ''}`;
  ...
  async function loadConfig() {
    state.config = await api('/api/config'); const select = $('provider-select'); clear(select);
    const paths = state.config.storage_paths;
    $('storage-paths').hidden = !paths;
    $('storage-data-directory').textContent = paths?.data_directory || '未读取到保存位置';
    $('storage-configuration-file').textContent = paths?.configuration_file || '未读取到配置位置';
    (state.config.providers || []).forEach(provider => { const option = node('option', '', `${provider.name}${provider.configured || provider.id === 'demo' ? '' : '（待配置）'}`); option.value = provider.id; select.append(option); });
    let preference = null; try { preference = JSON.parse((window.LiuyaoAndroid?.readPreference() || localStorage.getItem('liuyao-model-preference')) || 'null'); } catch (_) { /* Ignore obsolete local preferences. */ }
    const deepseek = state.config.providers?.find(p=>p.id==='deepseek');
    if (preference?.defaults !== 'deepseek-pro-max' && preference?.provider === 'deepseek'
        && ['deepseek-flash','deepseek-v4-flash','deepseek-chat','deepseek-reasoner'].includes(preference.model)
        && deepseek?.default_model === 'deepseek-v4-pro') preference = {...preference, model:deepseek.default_model};
    if ([...select.options].some(item => item.value === preference?.provider)) select.value = preference.provider;
    else if ([...select.options].some(item => item.value === state.config.default_provider)) select.value = state.config.default_provider;
    updateModels(preference?.model);
    storePreference();
  }
  async function analyzeSavedCase(caseId) {
    const provider = chosenProvider();
    if (!provider || (!provider.configured && provider.id !== 'demo') || !$('model-select').value) {
      $('settings-dialog').showModal(); banner('案例与补充说明已保存。请配置并选择模型后继续解卦，也可以明确选择离线演示。', 'warning'); return null;
    }
    const previousRun = state.runs.find(run => run.analysis_run_id === state.runId);
    const model = modelSettings(); storePreference(); showView('results'); window.ReportViews.show('answer');
    let response;
    try {
      response = await api(`/api/cases/${encodeURIComponent(caseId)}/analyze`, { method: 'POST', data: { ...model, expected_revision_seq: state.revision } });
    } catch (error) {
      // A lost acknowledgement must not trigger another paid request.
      const detail = await api(`/api/cases/${encodeURIComponent(caseId)}`).catch(()=>null);
      if (!detail?.active_job_id) throw error;
      response = {job_id:detail.active_job_id};
    }
    renderAnalysis(null, 'running');
    state.analyzing = true; setBusy(true); $('analysis-progress').hidden = false; $('analysis-progress-text').textContent = '正在准备分析…'; $('analysis-status').textContent = '分析中';
    persistDraft();
    window.AnalysisStatus.start(response.job_id,caseId); window.AnalysisStatus.focus();
    let job;
    try { job = await pollJob(response.job_id, caseId); }
    catch (error) {
    ...
    if (job.result || job.error) renderAnalysis({ ...job.result, error: job.result?.error || job.error }, job.status, job.analysis_run_id);
    if (job.status === 'failed') banner('');
    else banner(job.status === 'completed' || job.result?.display_report?.conclusion ? '解卦已保存。可以补充情况继续分析，或在后续反馈中记录实际进展。' : '本次分析已保存，请查看需要补充或尚未确定的部分。', job.status === 'completed' || job.result?.display_report?.conclusion ? '' : 'warning');
    await refreshHistory(); return job;
  }
  async function startAnalysis({ savedCase = false } = {}) {
    ...
    } catch (error) { banner(getError(error), 'error'); }
    finally { state.analyzing = false; setBusy(false); }
  }
  async function resumeJob(jobId, caseId) {
    if (state.analyzing) return;
    renderAnalysis(null, 'running');
    state.analyzing = true; setBusy(true); $('analysis-progress').hidden = false; $('analysis-progress-text').textContent = '已恢复正在进行的分析…';
    try {
  ...
  async function readJob(jobId, caseId) {
    ...
    const previous = currentJob; currentJob = job; window.AnalysisStatus.update(job);
    if (job.analysis_run_id && state.runId !== job.analysis_run_id) {
      const wasOpen=$('ai-raw-replies').open;
      state.runId = job.analysis_run_id; resetRawReplies();$('ai-raw-replies').open=wasOpen;
    ...
    if (job.preview_report && $('analysis-content').dataset.previewRun !== job.analysis_run_id) {
      ...
      window.ReportViews.render(job.preview_report,container);container.dataset.previewRun=job.analysis_run_id;
      $('analysis-status').textContent='已解卦 · 整理中';$('run-history').hidden=true;
    }
    if (previous?.reply_count !== job.reply_count || !state.rawReplyLoaded || Date.now()-lastRawCheck>4500 || !['queued','running'].includes(job.status)) loadRawReplies(true);
    return job;
  }
  async function pollJob(jobId, caseId) {
    ...
    for (;;) {
      try {
        const job = await readJob(jobId,caseId); failures = 0;
        if (!['queued', 'running'].includes(job.status)) return job;
      } catch (error) {
        failures++; window.AnalysisStatus.offline(getError(error));
        if (['SESSION_REQUIRED','NOT_FOUND','ACCESS_DENIED'].includes(error.code)) throw error;
      }
      await new Promise(resolve => {
        const timer=setTimeout(()=>{wakePoll=null;resolve();},failures?5000:1200);
  ...
  window.AnalysisStatus.bind({
    ...
    async refresh(){
      ...
      if (state.analyzing) { state.rawReplyLoaded=false;if(wakePoll)wakePoll(); return; }
      try {
      ...
      } catch(error){window.AnalysisStatus.offline(getError(error));}
    }
  });
  async function appendCaseEvent(kind, { reanalyze = false } = {}) {
    if (state.busy || !state.caseId) return;
    const field = $(kind === 'context' ? 'context-text' : 'feedback-text'); const text = field.value;
    if (!text.trim()) { field.focus(); banner(kind === 'context' ? '请先写下需要补充的说明。' : '请先写下实际进展或结果。', 'warning'); return; }
    const caseId = state.caseId; const payload = { text };
    if (kind === 'feedback') {
      if (state.runId) payload.analysis_run_id = state.runId;
      if ($('feedback-occurred')?.value) payload.occurred_at = new Date($('feedback-occurred').value).toISOString();
      payload.match_degree = $('feedback-match')?.value || 'pending';
      payload.user_notes = $('feedback-notes')?.value || '';
    }
    setBusy(true); banner('');
    try {
      const alreadySaved = kind === 'context' && state.pendingContext?.caseId === caseId && state.pendingContext.text === text;
      if (!alreadySaved) {
        await api(`/api/cases/${encodeURIComponent(caseId)}/${kind}`, { method: 'POST', data: payload });
      ...
      if (kind === 'feedback') { $('feedback-occurred').value=''; $('feedback-notes').value=''; $('feedback-match').value='pending'; }
      if (kind === 'context') state.pendingContext = null;
    } catch (error) {
      banner(`${getError(error)}${kind === 'context' && state.pendingContext?.caseId === caseId ? ' 补充说明已保存，可以重试解卦。' : ' 输入仍保留，请重试。'}`, 'error');
    } finally { state.analyzing = false; setBusy(false); }
  }

  buildInputs();
  showView('input');
  $('timezone-note').textContent = `使用当前浏览器时区：${Intl.DateTimeFormat().resolvedOptions().timeZone || '本地时区'}。`;
  ...
  $('profile-select')?.addEventListener('change', onInputChange);
  $('person-subject').addEventListener('change', () => { window.LiuyaoV5?.selectSelf(); updatePersonFields({ clearIrrelevant: true }); onInputChange(); });
  ...
  $('cast-now').addEventListener('click', () => { $('cast-time').value = localDateTime(new Date()); $('time-details').open = true; onInputChange(); });
  $('cast-clear').addEventListener('click', () => { $('cast-time').value = ''; onInputChange(); });
  ...
  document.addEventListener('visibilitychange', () => { if(!document.hidden&&wakePoll)wakePoll(); });
  ...
  $('case-form').addEventListener('submit', async event => {
    event.preventDefault(); if (state.busy) return; setBusy(true); banner('');
    try { await saveCase(); showView('results', { scroll: true }); window.ReportViews.show('chart'); banner('案例已保存，基础排盘已生成。'); }
    catch (error) { banner(getError(error), 'error'); }
  ...
  ['input', 'results', 'feedback'].forEach(name => {
    ...
    tab.addEventListener('keydown', event => {
      ...
      const names = state.caseId ? ['input', 'results', 'feedback'] : ['input']; const current = names.indexOf(name);
      const next = event.key === 'Home' ? 0 : event.key === 'End' ? names.length - 1 : (current + (event.key === 'ArrowRight' ? 1 : -1) + names.length) % names.length;
      event.preventDefault(); showView(names[next]); $(`tab-${names[next]}`).focus();
    });
  });
  ...
  window.addEventListener('beforeunload', event => { let dirty = false; try { dirty = hasEnteredData() && !inputsEqual(getInput(false), state.input); } catch (_) { dirty = true; } dirty = dirty || Boolean($('context-text').value.trim() || $('feedback-text').value.trim()); if (dirty) { event.preventDefault(); event.returnValue = ''; } });
  // Native state is app-private and independent of the ephemeral loopback port.
  function persistDraft(){
    if(!window.LiuyaoAndroid)return;
    const fields={};document.querySelectorAll('#case-form input,#case-form textarea,#case-form select,#context-text,#feedback-form input,#feedback-form textarea,#feedback-form select').forEach((el,index)=>{if(el.type==='file')return;fields[el.id||el.name+'::'+el.value]={value:el.value,checked:el.checked};});
    window.LiuyaoAndroid.saveDraft(JSON.stringify({caseId:state.caseId,fields,view:state.view}));
  }
  let draftTimer;
  document.addEventListener('input',()=>{if(window.LiuyaoAndroid){clearTimeout(draftTimer);draftTimer=setTimeout(persistDraft,300);}});
  document.addEventListener('change',()=>{if(window.LiuyaoAndroid){clearTimeout(draftTimer);draftTimer=setTimeout(persistDraft,300);}});
  ...
  Promise.allSettled([loadConfig(), refreshHistory()]).then(async results => {
    results.forEach(result => { if (result.status === 'rejected') banner(getError(result.reason), 'error'); });
    if(!window.LiuyaoAndroid)return;
    try{
      const draft=JSON.parse(window.LiuyaoAndroid.readDraft()||'null');if(!draft)return;
