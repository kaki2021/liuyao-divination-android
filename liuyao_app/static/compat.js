/* Load before every feature: Android's WebView updates independently of Android. */
(function () {
  'use strict';
  function install(target, name, value) {
    if (!target[name]) Object.defineProperty(target, name, {value: value, configurable: true, writable: true});
  }
  ...
  install(Object, 'hasOwn', function (object, key) { return Object.prototype.hasOwnProperty.call(object, key); });
  ...
  install(Promise, 'allSettled', function (values) {
    return Promise.all(Array.from(values).map(function (value) {
      return Promise.resolve(value).then(function (result) { return {status: 'fulfilled', value: result}; },
  ...
  install(Blob.prototype, 'arrayBuffer', function () {
    ...
    return new Promise(function (resolve, reject) {
      ...
      reader.onerror = function () { reject(reader.error || new Error('文件读取失败，请重新选择。')); };
      reader.readAsArrayBuffer(blob);
    });
  });
  function requestId() {
    // Cryptographic randomness is available on every supported WebView; randomUUID is newer.
  ...
  window.LiuyaoCompat = {requestId: requestId};
  function startupFailure() {
    var banner = document.getElementById('global-message');
    if (banner) {
      banner.textContent = '页面未能完整加载，请关闭应用后重新打开。已保存的案例仍保留。';
      banner.className = 'message-banner error'; banner.hidden = false;
    }
  }
  window.addEventListener('error', function () { if (!window.LiuyaoApp) startupFailure(); });
  // App forms must never navigate, even if a later script fails during initialization.
